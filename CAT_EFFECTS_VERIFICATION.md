...
- New ranks appear only inside the matching remapped starting house for three loaded seconds. Player cards display names and `Finished #1`, `Finished #2`, etc., with no cat artwork. A resolved rank cancels any obsolete capture/home sequence in that house. Final standings celebrate every house before opening text-only results; there are no cats on results cards. Completed-game resume uses static house cats and does not replay historical capture/finish effects. Rank cats require all four tokens to finish.
...
- All **18 production Chromium scenarios pass**, covering offline process restart, all routes/media/query strings, cache failures, safe updates, capture sequences for every colour, reduced motion, non-final home arrivals, live rank effects in 2P/3P/4P, both team outcomes, resumed static standings and responsive layouts.
