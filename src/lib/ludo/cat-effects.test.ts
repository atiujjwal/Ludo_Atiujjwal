...
describe("rank cat identity and timing", () => {
  ...
  it("keeps persistent ranks after loaded celebrations and restores them without countdowns", () => {
    vi.useFakeTimers();
    const effects = { show: vi.fn(), hide: vi.fn() };
    const loaded = vi.fn();
    const feedback = createRankFeedback({}, effects, { persistent: true, onLoaded: loaded });
    const shown = feedback.update({ red: "babsb-cat" });
    feedback.loaded("red", shown.red!.session);
    feedback.loaded("red", shown.red!.session);
    vi.advanceTimersByTime(60000);
    expect(loaded).toHaveBeenCalledExactlyOnceWith("red", shown.red!.session);
    expect(effects.hide).not.toHaveBeenCalled();
    expect(vi.getTimerCount()).toBe(0);
    feedback.update({ red: "babsb-cat" });
    expect(effects.show).toHaveBeenCalledTimes(1);
  ...
  it("restarts final persistent sessions, ignores obsolete loads and settles failed media", () => {
    vi.useFakeTimers();
    const effects = { show: vi.fn(), hide: vi.fn() };
    const loaded = vi.fn();
    const feedback = createRankFeedback({}, effects, { persistent: true, onLoaded: loaded });
    const old = feedback.update({ red: "babsb-cat" }).red!;
    const next = feedback.update({ red: "babsb-cat", blue: "crying-crying-cat" }, true);
    feedback.loaded("red", old.session);
    expect(loaded).not.toHaveBeenCalled();
    vi.advanceTimersByTime(10000);
    expect(loaded).toHaveBeenCalledWith("red", next.red!.session);
    expect(loaded).toHaveBeenCalledWith("blue", next.blue!.session);
    feedback.loaded("blue", next.blue!.session);
    expect(loaded).toHaveBeenCalledTimes(2);
    expect(effects.hide).not.toHaveBeenCalled();
    feedback.cancel("red");
    feedback.dispose();
    expect(effects.hide).toHaveBeenCalledTimes(2);
    expect(vi.getTimerCount()).toBe(0);
  });
});
